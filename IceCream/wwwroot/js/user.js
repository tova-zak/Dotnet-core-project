...
// הצג Toasts להודעות מערכת
// small toast helper (reused from myicecreams)
...
(function () {
    try {
        ...
        connection.on('Notify', (payload) => {
            console.log('SignalR Notify (user page) received', payload);
            try {
                const data = typeof payload === 'string' ? JSON.parse(payload) : payload;
                // simple behavior: if action targets this user, refresh
                const myId = getCurrentUserId();
                if (!myId) return;

                // handle new user added (admins should refresh users list)
                const claims = parseJwt(localStorage.getItem('token'));
                const isAdminUser = claims && claims.type === 'Admin';
                if (data && data.action === 'user_added') {
                    console.log('SignalR: user_added detected, refreshing users for admin=', isAdminUser);
                    if (isAdminUser) {
                ...
                if (data && data.action === 'user_deleted') {
                    // if the current user was deleted, show message and redirect
                    const deletedId = String(data.userId);
                    const myId = getCurrentUserId();
                    if (myId && String(myId) === deletedId) {
                        showToast('נמחקת מרשימת משתמשי האתר מסיבות השמורות עם המנהל');
                        setTimeout(() => {
                    ...
                    // otherwise admin should refresh the list
                    if (isAdminUser) getUsers();
                    return;
                }

                if (data && data.iceId) {
                    // refresh current profile/icecreams
                    showProfile(myId, { hideList: false });
                }
            } catch (e) {
                console.warn('Failed to parse SignalR payload (user page)', e);
            }
        });

        connection.start().then(() => console.log('SignalR (user) connected')).catch(err => console.error('SignalR (user) connection error', err));
        connection.onclose(err => console.warn('SignalR (user) closed', err));

    } catch (e) {
        console.warn('SignalR (user) setup failed', e);
    }
})();