...
public class OrderService:IOrderService
{
    ...
    public bool Update(int id, IceCreamModel newIceCream)
    {
        ...
        // notify clients that item updated
        try
        {
            var payload = JsonSerializer.Serialize(new { action = "ice_updated", id = id, item = newIceCream });
            Console.WriteLine($"[OrderService] Update: broadcasting update id={id}");
            // broadcast to all connected clients
            _ = _hubContext.Clients.All.SendAsync("Notify", payload);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[OrderService] Failed to notify hubs: {ex}");
        }
